...
<script type="text/javascript">
		...
		$.post("run.ht?isList=1&${pkVar}="+id,function(responseText){
			...
			if (obj.isSuccess()) {
				$.ligerDialog.success("启动流程成功！", "成功", function(rtn) {
					...
					window.location.href = "<#noparse>${ctx}</#noparse>/isunlandUI/${vars.projectName}/${system}/${package}/${classVar}/list.ht";
				});
			} else {
	...
	function startFlowForm(defId, businessKey){
		var url=__ctx +"/platform/bpm/task/startFlowForm.ht?defId="+defId+"&businessKey="+businessKey;
		url = url.getNewUrl();
		...
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<div class="group"><a class="link add" href="edit.ht?defId=<#noparse>${</#noparse>defId}"><span></span>添加</a></div>
					<div class="l-bar-separator"></div>
					<div class="group"><a class="link update" id="btnUpd" action="edit.ht?defId=<#noparse>${</#noparse>defId}"><span></span>修改</a></div>
					<#if !model.variables.flowKey?exists>
					<div class="l-bar-separator"></div>
		    ...
		    <display:table name="${classVar}List" id="${classVar}Item" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
				...
				<display:column title="管理" media="html" style="width:220px">
					...
					<c:if test="<#noparse>${</#noparse>${classVar}Item.runId==0}">
						<c:choose>
							<c:when test="<#noparse>${</#noparse>empty defId}">
								<a href="#" onclick="startFlow('<#noparse>${</#noparse>${classVar}Item.${pkVar}}')" class="link run"><span></span>提交</a>
							</c:when>
							<c:otherwise>
								<a href="#" onclick="startFlowForm(<#noparse>${</#noparse>defId}, '<#noparse>${</#noparse>${classVar}Item.${pkVar}}')" class="link run">启动流程</a>
							</c:otherwise>
						</c:choose>
					...
					<a href="edit.ht?${pkVar}=<#noparse>${</#noparse>${classVar}Item.${pkVar}}&defId=<#noparse>${</#noparse>defId}" class="link edit">编辑</a>
